...
const ModificarAlmacen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await updateAlmacen(id, almacen);
  ...
  const inputClasses =
    "w-full px-5 py-3 mb-10 border border-gray-400 rounded-full focus:outline-none focus:ring-2 focus:ring-red-800 appearance-none bg-white text-gray-700";
    ...
    <div className="flex flex-col justify-center items-center min-h-screen bg-gray-50 p-4 sm:p-10">
      <div className="w-full max-w-xl bg-white border border-gray-200 rounded-2rem shadow-lg p-6 sm:p-10 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-2">
          <label className="block ml-4 text-md font-bold text-gray-600">
            Dirección
          </label>
          <input
            type="text"
