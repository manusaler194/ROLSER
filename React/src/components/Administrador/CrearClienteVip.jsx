...
const CrearClienteVip = () => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                ...
                const resCatalogos = await axios.get('http://localhost/api/catalogo');
                const resComerciales = await axios.get('http://localhost/api/comerciales');

                // --- DEBUG: MIRA LA CONSOLA (F12) PARA VER LOS NOMBRES REALES ---
                console.log("EJEMPLO ADMIN:", resAdmins.data[0] || resAdmins.data.admin?.[0]);
                console.log("EJEMPLO CATALOGO:", resCatalogos.data[0] || resCatalogos.data.catalogo?.[0]);
                console.log("EJEMPLO COMERCIAL:", resComerciales.data[0] || resComerciales.data.comercial?.[0]);
                // -----------------------------------------------------------------
                ...
                const listaCatalogos = resCatalogos.data.catalogo || resCatalogos.data.catalogos || resCatalogos.data;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost/api/clientesVip/guardar', datosEnviados);
        ...
        <div className="flex justify-center items-center min-h-screen bg-gray-50 font-sans">
            <div className="w-full max-w-2xl p-10 bg-white border border-gray-200 rounded-[2rem] shadow-lg">
                ...
                <form onSubmit={handleSubmit}>
                    <input type="text" name="nombre" placeholder="Nombre Completo" value={clienteVip.nombre} className={inputClasses} onChange={handleChange} required />
                    <input type="text" name="telefono" placeholder="Teléfono" value={clienteVip.telefono} className={inputClasses} onChange={handleChange} required />
                    <input type="email" name="correo" placeholder="Correo Electrónico" value={clienteVip.correo} className={inputClasses} onChange={handleChange} required />
                    <input type="text" name="direccion" placeholder="Dirección" value={clienteVip.direccion} className={inputClasses} onChange={handleChange} required />

                    {/* SELECT ADMINISTRADOR */}
                    <div className="relative mb-6">
                        <label className="ml-4 mb-1 block text-xs font-bold text-gray-500 uppercase">Administrador</label>
                        <select name="id_administrador" value={clienteVip.id_administrador} className={inputClasses} onChange={handleChange}>
                    ...
                    {/* SELECT COMERCIAL */}
                    <div className="relative mb-6">
                        <label className="ml-4 mb-1 block text-xs font-bold text-gray-500 uppercase">Comercial</label>
                        <select name="id_comercial" value={clienteVip.id_comercial} className={inputClasses} onChange={handleChange}>
                            <option value="">-- Ninguno --</option>
                            {Array.isArray(comerciales) && comerciales.map((com) => {
                                // INTENTAMOS VARIOS NOMBRES POSIBLES
                                const nombreMostrar = com.nombre || com.name || `Comercial #${com.id}`;
                    ...
                    <div className="relative mb-8">
                        <label className="ml-4 mb-1 block text-xs font-bold text-gray-500 uppercase">Catálogo</label>
                        <select name="id_catalogo" value={clienteVip.id_catalogo} className={inputClasses} onChange={handleChange}>
                            <option value="">-- Ninguno --</option>
                            {Array.isArray(catalogos) && catalogos.map((cat) => {
                                // INTENTAMOS: nombre_temporada -> nombre -> title -> descripcion -> ID
                                const textoCatalogo = cat.nombre_temporada || cat.nombre || cat.name || cat.titulo || `Catálogo #${cat.id}`;
                                return (
                    ...
                    <button type="submit" className="w-full py-4 mt-2 text-white font-bold bg-[#b3002d] rounded-full hover:bg-red-900 transition-all transform active:scale-95 shadow-lg"> 
                        CREAR CLIENTE VIP
                    </button>
                </form>
            </div>
             <div className="absolute bottom-10 right-10 md:bottom-20 md:right-20">
                <button onClick={() => navigate('/usuarios')} className="bg-[#bc002d] text-white px-12 py-4 rounded-3xl text-2xl font-bold hover:bg-red-800 shadow-lg transition-transform active:scale-95 cursor-pointer">
                    Volver
                </button>
            </div>
        </div>
